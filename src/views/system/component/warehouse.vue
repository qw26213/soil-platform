  ...
  <div style="margin-top:-20px">
    <div class="filterDiv" style="padding-top:0px">
      <div class="width:100p%;">
        ...
        <!-- <br /> -->
        <el-button type="primary" class="saveBtn" size="mini" @click="save">保存并继续创建</el-button>
        <!-- <span class="warehouse">仓库名称：{{ deport_name }}</span> -->
      </div>
    </div>
    <div class="contentDiv">
      <el-table :cell-style="cellStyle" :data="tableData" resize :header-cell-style="headerCellStyle">
        <el-table-column prop="id" label="仓库名称" width="130">
          <template slot-scope="{row}">
            <input type="text" class="inputCell" v-model="row.deport_name" />
          </template>
        </el-table-column>
        <el-table-column prop="user_name" label="负责人" width="100" align="center">
          <template slot-scope="{row}">
            <input type="text" class="inputCell" v-model="row.depoter" />
          </template>
        </el-table-column>
        <el-table-column prop="user_phone" label="责任人电话" width="120" align="center">
          <template slot-scope="{row}">
            <input type="text" class="inputCell" v-model="row.depoter_phone" />
          </template>
        </el-table-column>
        ...
        <el-table-column prop="task_count" label="邮编" width="120" align="center">
          <template slot-scope="{row}">
            <input type="text" class="inputCell" v-model="row.deport_mailCode" />
          </template>
        </el-table-column>
        ...
        <el-table-column prop="task_perform_count" label="创建人" width="120" align="center">
          <template slot-scope="{row}">
            <input type="text" class="inputCell" v-model="row.creater" />
          </template>
        </el-table-column>
        <el-table-column prop="task_perform_count" label="创建日期" width="150" align="center">
          <template slot-scope="{row}">
