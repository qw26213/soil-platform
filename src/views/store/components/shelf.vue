        ...
        <el-table :data="tableData" style="width: 100%" :header-cell-style="headerCellStyle">
          ...
          <el-table-column width="80">
            <template slot-scope="{row}">
              <el-button type="text" @click="toPage(`./shelfdetail?shelf_code=${row.name}&deport_code=${row.deport_code}`)">查看详情</el-button>
            </template>
          </el-table-column>
        </el-table>
        <pagination v-if="totalPage>10" :total="totalPage" :page.sync="page" :limit.sync="pageSize" @pagination="getSearchShelf" />
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    async getDepotList() {
      ...
      this.getSearchShelf() //获取仓库查询table
    },
    // 仓库查询
    async getDeport_v2() {
