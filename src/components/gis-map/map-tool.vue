<template>
    <div>
        <ul class="map-tools">
            <li v-for="(item, index) in mapTools" :key="index" :class="item.id === mapTool ? 'active' : ''" @click="mapTool = item.id">
                <i :class="item.icon" />
            </li>
        </ul>
    </div>
...
<script>
import { get_point_result } from '@/api/detect'
export default {
    props: {
        map: {
            type: Object,
    ...
    methods: {
            ...
            const clusterFree = {
                ...
                layout: {
                    'icon-image': [
                        ...
                        ['get', 'status'],
                        'OUTTIME', 'marker-grey', //#aaaaaa
                        'FINISHED', 'marker-freedom', //#45e984
        ...
        addDrawLayer() {
            ...
            that.delMarker = new mapboxgl.Marker(delOption).setLngLat([0, 0]).addTo(map);
